...
class UserFollowService:
    ...
    def __init__(self, follower_user_id: int, target_user_id: int) -> None:
        ...
        self.follower_user = next((user for user in users if user.id == follower_user_id), None)
        self.target_user = next((user for user in users if user.id == target_user_id), None)
